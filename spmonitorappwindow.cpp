...
SPMonitorAppWindow::SPMonitorAppWindow(
  ...
  m_searchentry->signal_search_changed().connect(
      sigc::mem_fun(*this, &SPMonitorAppWindow::on_search_text_changed));
  m_stack->property_visible_child().signal_changed().connect(
      sigc::mem_fun(*this, &SPMonitorAppWindow::on_visible_child_changed));
  m_sidebar->property_reveal_child().signal_changed().connect(
      sigc::mem_fun(*this, &SPMonitorAppWindow::on_reveal_child_changed));
...
void SPMonitorAppWindow::on_search_text_changed() {
  ...
  if (!tab) {
    std::cout << "SPMonitorAppWindow::on_search_text_changed(): No visible "
                 "child."
              << std::endl;
    return;
  }
  ...
  if (!view) {
    std::cout
        << "SPMonitorAppWindow::on_search_text_changed(): No visible text view"
        << std::endl;
    return;
  }
...
void SPMonitorAppWindow::on_reveal_child_changed() { update_words(); }

void SPMonitorAppWindow::update_words() {
  auto tab = dynamic_cast<Gtk::ScrolledWindow *>(m_stack->get_visible_child());
  if (!tab) return;

  auto view = dynamic_cast<Gtk::TextView *>(tab->get_child());
  if (!view) {
    std::cout << "SPMonitorAppWindow::update_words(): No visible text view"
              << std::endl;
    return;
  }
  ...
  for (const auto &word : words) {
    ...
    row->signal_clicked().connect(sigc::bind(
